...
- **Одна доска на проект** — живёт в репо проекта в `echo/`, без мультитенантности; имя проекта из `BOARD_NAME` светится логотипом в шапке
- **Документы проекта внутри** — README и docs читаются живьём из того же репо, с поиском; список короткий, с бейджем формата
- **Ожидание заказчика** — дата запроса, дней в ожидании, подсветка залежей
- **Свои задачи со сроками** — WIP-лимит 1-2 под соло-режим
- **Файлы от любого участника** — картинки и pdf внутри, docx скачать + md рядом; скриншот из буфера вставляется через Ctrl+V
...
Направление правок одно: код правится в шаблоне, копии только получают обновления.
